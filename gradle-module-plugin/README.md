...
4. When depending on artifacts (dependencies) from the Ignition SDK, they should be specified as `compileOnly` or `compileOnlyApi`  dependencies as they will be provided by the Ignition platform at runtime.  Dependencies that are applied with either the `modlApi` or `modlImplementation` _Configuration_ in any subproject of your module will be collected and included in the final modl file, including transitive dependencies. Test and Compile-time dependencies should be specified in accordance with the best practices described in Gradle's `java-library` [plugin documentation](https://docs.gradle.org/current/userguide/java_library_plugin.html). This plugin diverges from Gradle's `java-library` semantics only where we resolve dependencies for inclusion into the modl.  Unlike the `java-library` plugin, transitive dependencies of `modlImplementation` configurations WILL be collected for use in the module. That one difference aside, all other implications apply as documented by the Gradle java-library plugin (e.g. - publishing, artifact uploading, transitive dependency handling, etc).  Test-only dependencies should NOT be marked with any `modl` configuration.
...
| Configuration  | Usage Suggestion | Included in Module? | Includes Transitive Dependencies? |
|-------|--------|-------------------------|----------|
| compileOnly   | Use for 'compile time only' dependencies, including ignition sdk dependencies. Similar to maven 'provided'.  | No | No |
| compileOnlyApi   | Use for 'compile time only' dependencies, including ignition sdk dependencies. Similar to maven 'provided'.  | No | Yes |
| api    | Project dependencies that do not explicitly get registered in the module DSL project scopes&#10013;  | No | Yes |
| implementation   | Project dependencies that do not explicitly get registered in the module DSL project scopes&#10013; | No | No |
| modlImplementation   | Dependencies that are used in a module project's implementation, but are not part of a public API  | Yes | No|
| modlApi | Dependencies that are used in a module project and are exposed to dependents&#10013;&#10013;  | Yes | Yes |
